...
function AdminPage() {
  ...
  const [companyEditData, setCompanyEditData] = useState<CompanySettings>({ id: 1, address: "", map_embed_url: "", phone1: "", phone2: "" });
...
useEffect(() => {
    ...
    async function loadData() {
      ...
      if (supabase) {
        try {
          const { data: linkData, error: linkError } = await supabase.from("social_links").select("*");
          if (linkData && !linkError) {
            setSocialLinks(linkData.map((l: any) => ({
              id: String(l.id),
              name: l.name,
              url: l.url,
              icon: l.icon,
  ...
  const handleSaveCompanySettings = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({ address: companyEditData.address, map_embed_url: companyEditData.map_embed_url, phone1: companyEditData.phone1, phone2: companyEditData.phone2 })
        .eq("id", 1);
  ...
  const handleAddProjectSubmit = async () => {
    const { name, description, category, status, userCount, uptime, github_link, live_link } = newProjectData;
    if (!name) {
    ...
    try {
      const { data, error } = await supabase
        .from("projects")
        .insert({ name, description, category, status, uptime, usercount: userCount, github_link, live_link })
        .select()
        .single();
      if (error) throw error;
      ...
      setNewProjectData({ name: "", description: "", category: "", status: "Active", userCount: "0", uptime: "0%", github_link: "", live_link: "" });
      ...
      <section className="container-page py-10 sm:py-14">
        ...
        <div className="mt-6 flex items-center gap-2 overflow-x-auto border-b border-border pb-3 scrollbar-none">
          {[
            ...
            { id: "inbox", label: `Inbox (${messages.filter((m) => m.status === "new").length})`, icon: Mail },
        ...
        {activeTab === "overview" && (
          <div className="mt-8 space-y-8">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { title: "Total Inquiries", value: messages.length.toString(), change: "From contact form", icon: Mail, color: "text-blue-600 bg-blue-50" },
              ].map((stat) => {
            ...
            <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
              <div className="rounded-3xl border border-border bg-surface p-6 shadow-soft">
                <div className="flex items-center justify-between border-b border-border/60 pb-4">
                  ...
                  <button type="button" onClick={() => setActiveTab("inbox")} className="text-xs font-semibold text-primary hover:underline flex items-center gap-1">
              ...
              <div className="rounded-3xl border border-border bg-surface p-6 shadow-soft">
                <h3 className="font-display text-lg font-bold text-ink border-b border-border/60 pb-4">System Quick Controls</h3>
                <div className="mt-4 space-y-3">
                  <a href="https://dashboard.emailjs.com/" target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-2xl border border-border p-3.5 hover:border-primary transition-colors text-xs font-bold text-ink">
                    <span>EmailJS Service Panel</span>
                    <ArrowUpRight size={14} className="text-muted-foreground" />
                  </a>
                  <a href="https://supabase.com/dashboard/project/yayoyrouufztwxygsuph" target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-2xl border border-border p-3.5 hover:border-primary transition-colors text-xs font-bold text-ink">
                    <span>Supabase Auth & Database</span>
                    <ArrowUpRight size={14} className="text-muted-foreground" />
                  </a>
                  <a href="https://github.com/ma-nees/QubixTechNepal" target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-2xl border border-border p-3.5 hover:border-primary transition-colors text-xs font-bold text-ink">
                ...
                <div>
                  <div className="flex items-start justify-between border-b border-border/60 pb-4">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button type="button" onClick={() => toggleStatus(selectedMessage.id, "replied")} className="rounded-xl border border-border bg-background px-3 py-1.5 text-xs font-bold text-emerald-700 hover:bg-emerald-50">Mark Replied</button>
                      <button type="button" onClick={() => toggleStatus(selectedMessage.id, "archived")} className="rounded-xl border border-border bg-background px-3 py-1.5 text-xs font-bold text-gray-700 hover:bg-gray-100">Archive</button>
                    </div>
                  </div>
                  <div className="mt-6 space-y-4">
                    <div className="rounded-2xl border border-border/80 bg-background p-5 text-sm leading-relaxed text-ink">{selectedMessage.message}</div>
                  </div>
                </div>
              ) : (
                <div className="grid h-64 place-items-center text-xs text-muted-foreground">Select a message to view details</div>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === "projects" && (
          <div className="mt-8">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {projects.map((proj) => (
                <div key={proj.id} className="rounded-3xl border border-border bg-surface p-4 shadow-soft">
                  ...
                  <div className="mt-2 flex gap-3 text-xs">
                    {proj.github_link && (
                      <a href={proj.github_link} target="_blank" rel="noreferrer" className="text-primary hover:underline flex items-center gap-1">
                        GitHub <ArrowUpRight size={12} />
                      </a>
                    )}
                    {proj.live_link && (
                      <a href={proj.live_link} target="_blank" rel="noreferrer" className="text-primary hover:underline flex items-center gap-1">
                  ...
                  <div className="mt-3 flex gap-2">
                    <button className="rounded bg-amber-500 px-2 py-1 text-xs text-white" onClick={() => editProject(proj.id)}>
                      Edit
                    </button>
                    <button className="rounded bg-destructive px-2 py-1 text-xs text-white" onClick={() => deleteProject(proj.id)}>
                      Delete
                    </button>
                  </div>
                </div>
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <div className="grid gap-4">
                <div>
                  <label className="block text-xs font-semibold text-ink mb-1.5">Project Name *</label>
                  <input
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewProjectData(prev => ({ ...prev, status: e.target.value }))}
                  >
                ...
                <div>
                  <label className="block text-xs font-semibold text-ink mb-1.5">GitHub Link (Optional)</label>
                  <input
                    className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all"
                    placeholder="https://github.com/..."
                    value={newProjectData.github_link}
                    onChange={(e) => setNewProjectData(prev => ({ ...prev, github_link: e.target.value }))}
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-ink mb-1.5">Live Link (Optional)</label>
                  <input
                    className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all"
                    placeholder="https://..."
                    value={newProjectData.live_link}
                    onChange={(e) => setNewProjectData(prev => ({ ...prev, live_link: e.target.value }))}
                  />
                </div>
        ...
        {showAddProject && (
          <div className="fixed inset-0 bg-black/30 z-40" onClick={() => setShowAddProject(false)} />
        )}
                ...
                <div className="grid gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditingProject(prev => prev ? { ...prev, github_link: e.target.value } : null)}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-ink mb-1.5">Live Link (Optional)</label>
                    <input
                      ...
                      onChange={(e) => setEditingProject(prev => prev ? { ...prev, live_link: e.target.value } : null)}
                    />
                  </div>
  ...
  <div className="mt-8">
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {socialLinks.map((link) => (
        <div key={link.id} className="rounded-3xl border border-border bg-surface p-4 shadow-soft flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="min-w-0 flex-1 overflow-hidden">
            <div className="font-medium text-ink mb-1">{link.name}</div>
            <a href={link.url} className="text-primary hover:underline text-sm truncate block" target="_blank" rel="noreferrer">{link.url}</a>
          </div>
          <div className="flex gap-2 shrink-0">
            <button className="rounded bg-amber-500 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-amber-600" onClick={() => editSocialLink(link.id)}>
              Edit
            </button>
            <button className="rounded bg-destructive px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-red-700" onClick={() => deleteSocialLink(link.id)}>
              Delete
            </button>
          </div>
        </div>
...
{/* Tab 4: Settings & Health */}
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-3xl border border-border bg-surface p-6 shadow-soft">
              <div className="flex items-center justify-between border-b border-border/60 pb-4">
                ...
                {!isEditingCompany && (
                  <button onClick={() => setIsEditingCompany(true)} className="text-xs font-semibold text-primary hover:underline">Edit</button>
                )}
              </div>
              ...
              <div className="mt-6 text-sm">
                {isEditingCompany ? (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-ink mb-1.5">Company Address</label>
                      <input 
                        className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary"
                        value={companyEditData.address}
                        onChange={(e) => setCompanyEditData(prev => ({ ...prev, address: e.target.value }))}
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-ink mb-1.5">Google Maps Embed URL</label>
                      <input 
                        className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary"
                        value={companyEditData.map_embed_url}
                        onChange={(e) => setCompanyEditData(prev => ({ ...prev, map_embed_url: e.target.value }))}
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-ink mb-1.5">Primary Phone</label>
                        <input 
                          className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary"
                          value={companyEditData.phone1 || ""}
                          onChange={(e) => setCompanyEditData(prev => ({ ...prev, phone1: e.target.value }))}
                          placeholder="+977 98..."
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-ink mb-1.5">Secondary Phone</label>
                        <input 
                          className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary"
                          value={companyEditData.phone2 || ""}
                          onChange={(e) => setCompanyEditData(prev => ({ ...prev, phone2: e.target.value }))}
                          placeholder="+977 98..."
                        />
                      </div>
                    </div>
                    <div className="flex gap-2 pt-2">
                      ...
                      <button 
                        onClick={() => { setIsEditingCompany(false); setCompanyEditData(companySettings || { id: 1, address: "", map_embed_url: "", phone1: "", phone2: "" }); }}
                  ...
                  <div className="space-y-4 text-xs text-ink">
                    <div className="rounded-2xl border border-border p-4 bg-background">
                      <div className="font-semibold text-muted-foreground mb-1">Company Address</div>
                      <div className="font-medium text-ink">{companySettings?.address || "Not set"}</div>
                    </div>
                    <div className="rounded-2xl border border-border p-4 bg-background">
                      <div className="font-semibold text-muted-foreground mb-1">Map Embed URL</div>
                      <div className="font-medium text-ink truncate" title={companySettings?.map_embed_url}>{companySettings?.map_embed_url || "Not set"}</div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="rounded-2xl border border-border p-4 bg-background">
                        <div className="font-semibold text-muted-foreground mb-1">Primary Phone</div>
                        <div className="font-medium text-ink">{companySettings?.phone1 || "Not set"}</div>
                      </div>
                      <div className="rounded-2xl border border-border p-4 bg-background">
                        <div className="font-semibold text-muted-foreground mb-1">Secondary Phone</div>
                        <div className="font-medium text-ink">{companySettings?.phone2 || "Not set"}</div>
                      </div>
                    </div>
                  </div>
                )}
        ...
        >
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <div className="grid gap-4">
                <div>
                  <label className="block text-xs font-semibold text-ink mb-1.5">Platform Name *</label>
                  <select
                    className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary transition-all"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewSocialLinkData(prev => ({ ...prev, url: e.target.value }))}
                  />
                </div>
              </div>
            </div>
        ...
        {showAddSocialLink && (
          <div className="fixed inset-0 bg-black/30 z-40" onClick={() => setShowAddSocialLink(false)} />
        )}
        ...
        {editingSocialLink && (
          <div className="fixed inset-0 bg-black/30 z-40" onClick={() => setEditingSocialLink(null)} />
        )}
      ...
      {toast && (
        ...
        >
          ...
          <button type="button" onClick={() => setToast(null)} className="ml-2 opacity-60 hover:opacity-100">✕</button>
        </div>
      )}
    </main>
  );
