...
const message = encodeURIComponent("Hello Qubix Tech Nepal! I'd like to discuss a project.");
...
export function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2 sm:bottom-6 sm:right-6">
      {open && (
        <div className="mb-1 flex max-w-[calc(100vw-2rem)] flex-col gap-1.5 rounded-2xl border border-border bg-surface p-2.5 shadow-xl sm:p-3">
          <p className="px-1 text-xs font-semibold text-ink">Chat with us</p>
          {contacts.map((c) => (
            <a
              key={c.phone}
              href={`https://wa.me/${c.phone}?text=${message}`}
              target="_blank"
      ...
      <button
        ...
        className="grid size-12 place-items-center rounded-full bg-[#25D366] text-white shadow-lg transition-transform hover:scale-105 active:scale-95 sm:size-14"
      >
        {open ? (
